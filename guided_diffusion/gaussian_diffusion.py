...
class GaussianDiffusion:
    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            out = self.p_sample(x=img, t=time, model=model, lam=lam)
            # Give condition.

            noisy_measurement = self.q_sample(measurement, t=time)

            if diffusion_whether == 0:
                img, distance_a = measurement_cond_fn(x_t=img,
                                        ...
                                        x_0_hat=img,
                                        lam=lam,
                                        fig_num = int((1000-idx)/20))
            else:
            # TODO: how can we handle argument for different condition method?
                img, distance_a = measurement_cond_fn(x_t=out['sample'],
                                        measurement=measurement,
                                        measurement_f=measurement_f, 
                                        save_root=save_root,
                                        noisy_measurement=noisy_measurement,
                                        ...
                                        lam=lam, 
                                        fig_num = int((1000-idx)/20))#pred_xstart

            img = img.detach()
            distance_f = torch.linalg.norm(torch.relu(img*1500+3000)-(1500*measurement_f+3000))
            distance_re = distance_f/torch.linalg.norm(1500*measurement_f+3000)
            diff_A.append(distance_a.cpu().detach().numpy())
            diff_f.append(distance_f.cpu().detach().numpy())
           
            pbar.set_postfix({'distance': distance_re.item()}, refresh=False)
            '''if record:
                os.makedirs(os.path.join(save_root, 'process', 'data'), exist_ok=True)
                if idx % 10 == 0:
                    file_path = os.path.join(save_root, 'process', 'data', f"x_{str(idx).zfill(4)}.npy")
                    img2 = img.cpu().detach().numpy()
                    np.save(file_path, img2)'''

            with open(os.path.join(save_root, 'text', 'distance_record.txt'), 'a') as op:
                op.write('\nprocess stop, relative distance is {}, and the step is {}\n'.format(distance_re, str(len(pbar)-idx)))
                if distance_a <= tau * delta or distance_re <= barrier:
                    op.write('end here\n')
                    # break
        plt.subplot(121)
        plt.plot(list(range(len(diff_A))),diff_A)
        plt.title('dis_A-epoch')
        plt.ylabel('dis_A')
        plt.subplot(122)
        plt.plot(list(range(len(diff_f))),diff_f)
        plt.title('dis_f-epoch', y=-0.3)
        plt.ylabel('dis_f')
        plt.savefig(os.path.join(save_root, 'figure', 'distance.png'))
        distance_all=[diff_A, diff_f]
        np.save(os.path.join(save_root, 'data', 'distance_af.npy'), distance_all)
        return img       
        
    def p_sample(self, model, x, t):
        raise NotImplementedError

    def p_mean_variance(self, model, x, t, lam):
        ...
        model_mean, pred_xstart = self.mean_processor.get_mean_and_xstart(x, t, model_output*(1+lam**2)/2)
...
class DDPM(SpacedDiffusion):
    def p_sample(self, model, x, t, lam):
        out = self.p_mean_variance(model, x, t, lam)
        sample = out['mean']

        noise = lam * torch.randn_like(x)
